...
namespace elFinder.NetCore.Drivers.AzureStorage
{
	...
	public class AzureStorageDriver : BaseDriver, IDriver
	{
		...
		public async Task<JsonResult> UploadAsync(FullPath path, IEnumerable<IFormFile> files, bool? overwrite, IEnumerable<FullPath> uploadPaths, IEnumerable<string> renames, string suffix)
		{
			...
			{
				var destination = uploadPaths.Count() > i ? uploadPaths.ElementAt(i).Directory.FullName : path.Directory.FullName;
				var f = new AzureStorageFile(AzureStorageAPI.PathCombine(destination, Path.GetFileName(file.FileName)));

				if (await f.ExistsAsync)
				{
					if (overwrite ?? path.RootVolume.UploadOverwrite)
					{
						await f.DeleteAsync();
						await AzureStorageAPI.UploadAsync(file, f.FullName);
						response.Added.Add((FileModel)await BaseModel.CreateAsync(this, new AzureStorageFile(f.FullName), path.RootVolume));
					}
					else
					{
						var newName = await CreateNameForCopy(f, suffix);
						await AzureStorageAPI.UploadAsync(file, AzureStorageAPI.PathCombine(f.DirectoryName, newName));
		...
		private async Task<SizeResponseModel> DirectorySizeAndCount(IDirectory d)
		{
			...
			// Add file sizes.
			foreach (IFile file in await d.GetFilesAsync())
			{
				response.FileCount++;
			...
			foreach (IDirectory di in await d.GetDirectoriesAsync())
			{
				response.DirectoryCount++;

				var subdir = await DirectorySizeAndCount(di);
		...
		private async void RemoveThumbsAsync(FullPath path)
		{
			if (path.IsDirectory)
			{
