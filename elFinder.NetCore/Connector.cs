	...
	public class Connector
	{
		...
		public async Task<IActionResult> GetThumbnailAsync(HttpRequest request, HttpResponse response, string hash)
		{
			if (hash != null)
			{
				...
				{
					if (await HttpCache.IsFileFromCacheAsync(path.File, request, response))
					{
						response.ContentType = Mime.GetMimeType(path.RootVolume.PictureEditor.ConvertThumbnailExtension(path.File.Extension));
					}
					else
					{
